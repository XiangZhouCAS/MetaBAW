...
ISOLATED_TOOLS: dict[str, IsolatedToolSpec] = {
    ...
    "binchicken": IsolatedToolSpec(
        key="binchicken",
        display_name="Bin Chicken",
        executable="binchicken",
        package="binchicken=0.14.1",
        python_version="3.11",
        default_environment="metabaw-binchicken-py311",
        option="--binchicken-env",
        purpose="Bin Chicken coassembly planning (Aviary is never invoked)",
...
SOFTWARE: dict[str, SoftwareRequirement] = {
    ...
    "megahit": SoftwareRequirement(
        "megahit",
        "megahit",
        "MetaBAW short-read coassembly after Bin Chicken planning",
    ),
}
...
def bin_requirements(args: object) -> list[SoftwareRequirement]:
    ...
    if getattr(args, "assembly_strategy", "default") == "bin-chicken":
        # Bin Chicken 0.14.1 conflicts with GTDB-Tk 2.7.2 in the main
        # environment and is therefore validated as an isolated tool.
        names.append("megahit")
...
def normalize_isolated_environment(key: str, environment: str) -> str:
    """Replace obsolete MetaBAW-managed environment names with current defaults."""
    if key not in ISOLATED_TOOLS:
        raise ValueError(f"Unknown isolated tool: {key}")
    normalized = environment.strip()
    basename = Path(normalized.rstrip("/\\")).name
    legacy_names = {
        "binchicken": {"metabaw-binchicken-py310"},
    }
    if basename in legacy_names.get(key, set()):
        return ISOLATED_TOOLS[key].default_environment
    return normalized
...
def configured_binchicken_singlem_metapackage() -> Path | None:
    """Return the reusable SingleM metapackage required for planning."""
    configured = (
        os.environ.get("SINGLEM_METAPACKAGE_PATH")
        or _runtime_config().get("binchicken_singlem_metapackage")
    )
    if not configured:
        return None
    return Path(str(configured)).expanduser().resolve()
...
def save_binchicken_singlem_metapackage(path_value: Path) -> Path:
    """Persist the SingleM metapackage location without installing Aviary."""
    normalized = path_value.expanduser().resolve()
    path = isolated_environment_config_path()
    config = _runtime_config()
    config["binchicken_singlem_metapackage"] = str(normalized)
    config["version"] = 1
    path.parent.mkdir(parents=True, exist_ok=True)
    temporary = path.with_suffix(path.suffix + ".tmp")
    temporary.write_text(
...
def isolated_environment_status(
    ...
    elif spec.key == "binchicken":
        package_version = _conda_package_version(executable, "binchicken")
        if package_version != "0.14.1":
            error = (
                f"Bin Chicken {package_version or 'unknown'} is installed; "
                "MetaBAW requires version 0.14.1"
            )
    return IsolatedEnvironmentStatus(
        spec,
        environment,
...
def install_isolated_cuda_runtime(
    ...
    if process.returncode != 0:
        ...
        print(
            "[WARNING] Conda could not solve the CUDA profile; retrying with "
            "the official PyTorch CUDA wheel.",
            flush=True,
        )
        pip_process = subprocess.run(pip_command, check=False)
        if pip_process.returncode != 0:
            raise RuntimeError(
                "CUDA-enabled PyTorch installation failed with both Conda and pip; "
...
def install_main_cuda_runtime(frontend: str | None = None) -> None:
    ...
    if process.returncode != 0:
        ...
        print(
            "[WARNING] Conda could not solve the CUDA profile; retrying with "
            "the official PyTorch CUDA 11.8 wheel.",
            flush=True,
        )
        pip_process = subprocess.run(pip_command, check=False)
        if pip_process.returncode != 0:
...
def reference_package_valid(path: Path) -> bool:
    """Accept a non-empty reference-package file or an unpacked directory."""
    return _nonempty_file(path) or _directory_has_nonempty_file(path)
