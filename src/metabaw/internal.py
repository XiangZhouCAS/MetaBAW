...
def _gtdbtk_temp_environment() -> tuple[dict[str, str], Path | None]:
    """Use a short POSIX alias when multiprocessing socket paths would overflow."""
    environment = os.environ.copy()
    if os.name != "posix":
        return environment, None
    configured = Path(environment.get("TMPDIR", "/tmp")).expanduser().resolve()
    if not _multiprocessing_socket_path_too_long(configured):
        return environment, None

    target = configured / "gtdbtk_multiprocessing"
    target.mkdir(parents=True, exist_ok=True)
    digest = hashlib.sha256(os.fsencode(str(target))).hexdigest()[:8]
    alias: Path | None = None
    for attempt in range(100):
        candidate = Path("/tmp") / f"mbw-{os.getpid()}-{digest}-{attempt}"
        try:
    ...
    if alias is None:
        ...
        print(
            "[WARNING] GTDB-Tk multiprocessing requires a shorter temporary path; "
            "the configured path could not be aliased, so transient IPC files will "
            "use /tmp.",
            flush=True,
        )
        return environment, None

    short_path = str(alias)
    environment.update({"TMPDIR": short_path, "TMP": short_path, "TEMP": short_path})
    print(
        f"[INFO] GTDB-Tk multiprocessing is using short temporary alias {alias}; "
        f"files remain under {target}.",
        flush=True,
    )
...
def run_gtdbtk_classify(
    ...
    environment, temp_alias = _gtdbtk_temp_environment()
    try:
        ...
        if temp_alias is not None:
            try:
                ...
                print(
                    f"[WARNING] Could not remove GTDB-Tk temporary alias "
                    f"{temp_alias}: {exc}",
                    flush=True,
                )
...
def report_coassembly_provenance(
    ...
    for value in coassemblies:
        try:
            ...
            assembly_samples, recovery_samples = raw_sets.split("|", 1)
        ...
        definitions[identifier] = (assembly_samples, recovery_samples)
    ...
    with output.open("w", encoding="utf-8", newline="") as handle:
        ...
        for fasta in files:
            ...
            if matching:
                ...
                assembly_samples, recovery_samples = definitions[identifier]
                writer.writerow(
                    (
                        fasta.name,
                        "bin_chicken_coassembly",
                        identifier,
                        assembly_samples,
                        recovery_samples,
                    )
...
def classify_niche_literature(
    ...
    if method not in {"cv", "prevalence"}:
    ...
    with abundance.open("r", encoding="utf-8-sig", newline="") as handle:
        ...
        for sample in sample_to_dataset:
            ...
            relative = sample_columns.get("relative_abundance")
            count = sample_columns.get("count")
            if relative is None or count is None:
                raise ValueError(
                    f"CoverM matrix lacks relative_abundance or count for {sample} "
    ...
    for taxon in sorted(taxon_members):
        ...
        eligible = total_reads >= min_total_reads and (
    ...
    for row in rows:
        if not row["eligible"]:
            ...
            reason = (
                "below_min_total_reads"
                if method == "cv"
                else "below_min_total_reads_or_prevalence"
            )
        elif method == "cv":
...
def build_parser() -> argparse.ArgumentParser:
    ...
    command.add_argument("--method", choices=("cv", "prevalence"), required=True)
...
def main(argv: list[str] | None = None) -> None:
    ...
    dispatch = {
        ...
        "format-functional-annotations": lambda: format_functional_annotations(
            args.input, args.gene_map, args.output, args.kind
        ),
